...
class DCE_Extension_Prototype
{
    ...
    public static function satisfy_dependencies($ret = \false, $deps = array())
    {
        ...
        if (!empty($deps)) {
            ...
            foreach ($deps as $pkey => $plugin) {
                if (!\is_numeric($pkey)) {
                    if (!\DynamicContentForElementor\Extensions\Helper::is_plugin_active($pkey)) {
                        $isActive = \false;
                    }
                } else {
                    if (!\DynamicContentForElementor\Extensions\Helper::is_plugin_active($plugin)) {
                        $isActive = \false;
                    }
                }
                if (!$isActive) {
    ...
    {
        $element->start_controls_section($section_name, ['tab' => Controls_Manager::TAB_ADVANCED, 'label' => '<span class="color-dce icon icon-dyn-logo-dce pull-right ml-1"></span> ' . $this->name]);
