...
class DynamicVisibility extends \DynamicContentForElementor\Extensions\DCE_Extension_Prototype
{
    ...
    public function get_control_section($section_name, $element)
    {
        ...
        $element->start_controls_section($section_name, ['tab' => 'dce_' . $low_name, 'label' => '<span class="color-dce icon icon-dyn-logo-dce pull-right ml-1"></span> ' . $this->name]);
    ...
    private function add_controls($element, $args)
    {
        ...
        if ($section == 'user') {
            ...
            $element->add_control('dce_visibility_usermeta', ['label' => __('User Field', 'dynamic-content-for-elementor'), 'type' => 'ooo_query', 'placeholder' => __('Meta key or Name', 'dynamic-content-for-elementor'), 'label_block' => \true, 'query_type' => 'fields', 'object_type' => 'user', 'description' => __('Triggered by a selected User Field value', 'dynamic-content-for-elementor'), 'separator' => 'before']);
        ...
        if ($section == 'post') {
            ...
            $element->add_control('dce_visibility_tax', ['label' => __('Taxonomy', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT2, 'options' => $taxonomies, 'description' => __('Triggered if current post is related to this Taxonomy', 'dynamic-content-for-elementor'), 'multiple' => \false, 'separator' => 'before']);
        ...
        if ($section == 'archive') {
            ...
            $element->add_control('dce_visibility_archive_tax', ['label' => __('Taxonomy', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT2, 'options' => $taxonomies, 'description' => __('Triggered if current post is related to this Taxonomy.', 'dynamic-content-for-elementor'), 'multiple' => \false, 'separator' => 'before', 'condition' => ['dce_visibility_archive' => 'is_tax']]);
            foreach ($taxonomies as $tkey => $atax) {
                if ($tkey) {
                    switch ($tkey) {
                        case 'post_tag':
