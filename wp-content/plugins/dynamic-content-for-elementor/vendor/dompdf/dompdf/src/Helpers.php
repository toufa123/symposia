...
class Helpers
{
    ...
    public static function parse_data_uri($data_uri)
    {
        ...
        $result = array('charset' => $match['charset'] ? $match['charset'] : 'US-ASCII', 'mime' => $match['mime'] ? $match['mime'] : 'text/plain', 'data' => $match['base64'] ? \base64_decode($match['data']) : $match['data']);
        return $result;
    }
    /**
    ...
    public static function explode_url($url)
    {
        ...
        if (isset($arr["scheme"]) && $arr["scheme"] !== "file" && \strlen($arr["scheme"]) > 1) {
            ...
            $protocol = "";
            // "file://"; ? why doesn't this work... It's because of
        ...
        if ($width == null || $height == null) {
            ...
            if (\substr($data, 0, 2) === "BM") {
                ...
                if (\strpos($data, "<svg") !== \false) {
                    $doc = new \DynamicOOOS\Svg\Document();
