...
use DOMDocument;
use DOMNode;
use DynamicOOOS\Dompdf\Adapter\CPDF;
use DOMXPath;
use DynamicOOOS\Dompdf\Frame\Factory;
use DynamicOOOS\Dompdf\Frame\FrameTree;
use DynamicOOOS\HTML5_Tokenizer;
use DynamicOOOS\HTML5_TreeBuilder;
use DynamicOOOS\Dompdf\Image\Cache;
use DynamicOOOS\Dompdf\Renderer\ListBullet;
use DynamicOOOS\Dompdf\Css\Stylesheet;
/**
...
class Dompdf
{
     ...
     * @deprecated
     */
    public static $native_fonts = array("courier", "courier-bold", "courier-oblique", "courier-boldoblique", "helvetica", "helvetica-bold", "helvetica-oblique", "helvetica-boldoblique", "times-roman", "times-bold", "times-italic", "times-bolditalic", "symbol", "zapfdinbats");
    /**
     * The list of built-in fonts
     *
     * @var array
     */
    public static $nativeFonts = array("courier", "courier-bold", "courier-oblique", "courier-boldoblique", "helvetica", "helvetica-bold", "helvetica-oblique", "helvetica-boldoblique", "times-roman", "times-bold", "times-italic", "times-bolditalic", "symbol", "zapfdinbats");
    /**
     * Class constructor
     *
    ...
    public function loadHtml($str, $encoding = 'UTF-8')
    {
        ...
        if (($file_encoding = \mb_detect_encoding($str, null, \true)) === \false) {
            $file_encoding = "auto";
        }
        ...
        $quirksmode = \false;
        if ($this->options->isHtml5ParserEnabled() && \class_exists("DynamicOOOS\\HTML5_Tokenizer")) {
            ...
            foreach ($this->tree->get_frames() as $frame) {
                ...
                if ($frame->get_node()->nodeName == "dompdf_generated") {
                    ...
                    $decoded_string = \preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/", function ($matches) {
                        return \DynamicOOOS\Dompdf\Helpers::unichr(\hexdec($matches[1]));
                    }, $style->content);
    ...
    private function write_log()
    {
        ...
        $time = (\microtime(\true) - $this->startTime) * 1000;
    ...
    public function getPaperSize($paperSize = null)
    {
        ...
        if (\is_array($size)) {
            ...
            if (isset(Adapter\CPDF::$PAPER_SIZES[\mb_strtolower($size)])) {
