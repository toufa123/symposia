...
trait Request
{
    ...
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            ...
            throw new \DynamicOOOS\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    /**
    ...
    {
        $opts = \DynamicOOOS\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \DynamicOOOS\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
