...
class Categories extends Powerpack_Widget {
	...
	public function slider_settings() {
		...
		$slider_options['breakpoints'] = array(
			$bp_desktop => array(
				'slidesPerView' => ( isset( $settings['columns'] ) && $settings['columns'] ) ? absint( $settings['columns'] ) : 3,
				'spaceBetween'  => ( isset( $settings['column_gap']['size'] ) && $settings['column_gap']['size'] ) ? $settings['column_gap']['size'] : 10,
			),
			$bp_tablet  => array(
				'slidesPerView' => ( isset( $settings['columns_tablet'] ) && $settings['columns_tablet'] ) ? absint( $settings['columns_tablet'] ) : 2,
				'spaceBetween'  => ( isset( $settings['column_gap_tablet']['size'] ) && $settings['column_gap_tablet']['size'] ) ? $settings['column_gap_tablet']['size'] : 10,
			),
			$bp_mobile  => array(
				'slidesPerView' => ( isset( $settings['columns_mobile'] ) && $settings['columns_mobile'] ) ? absint( $settings['columns_mobile'] ) : 1,
				'spaceBetween'  => ( isset( $settings['column_gap_mobile']['size'] ) && $settings['column_gap_mobile']['size'] ) ? $settings['column_gap_mobile']['size'] : 10,
			),
		);
					...
					<?php
						...
						if ( $all_categories ) {
							foreach ( $all_categories as $index => $cat ) {
								...
								?>
								<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'grid' ) ); ?> id="pp-cat-<?php echo esc_attr( $cat->term_id ); ?>">
