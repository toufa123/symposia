...
define( 'PREMIUM_TRUSTPILOT_API', 'https://appfb.premiumaddons.com/wp-json/fbapp/v2/trustapp' );
 ...
 */
function premium_trustpilot_data( $business_name, $api ) {

	$business_name = '/' . str_replace( ' ', '%20', $business_name );

	$url = PREMIUM_TRUSTPILOT_API . $api . $business_name;
...
function premium_trustpilot_reviews_business( $result, $settings ) {
	...
	<div class="premium-fb-rev-page-right">
		...
		<?php if ( isset( $result->name ) && $settings['show_name'] ) : ?>
			<div class="premium-fb-rev-page-link-wrapper">
				<?php
					$result->name  = '1' === $settings['api'] ? $result->name : $settings['name'];
		...
		<?php if ( isset( $result->reviews->count ) && 'yes' === $reviews_number ) : ?>
			<div class="premium-fb-rev-rating-count">
				<span><?php echo wp_kses_post( sprintf( $settings['number_text'], $result->reviews->count ) ); ?></span>
			</div>
		<?php endif; ?>
 ...
 * @param object $settings widget settings.
 */
function premium_trustpilot_old_api_reviews( $reviews, $settings ) {
...
/**
 * Render Trustpilot Reviews Layout for the new API.
 *
 * @since 2.5.2
 *
 * @param array  $reviews Trustpilot Reviews.
 * @param object $settings widget settings.
 */
function premium_trustpilot_new_api_reviews( $reviews, $settings ) {
	...
	<div class="premium-fb-rev-reviews">
		...
		if ( count( $reviews ) > 0 ) {
			...
			foreach ( $reviews as $review ) {
				...
				// $review->reviewer_url = 'https://trustpilot.com/users/' . $review->id;

				if ( $settings['show_image'] ) {
					if ( strlen( $review->reviewer_picture_url ) > 0 && false === strpos( $review->reviewer_picture_url, 'default-avatar' ) ) {
						$author_photo = '<img class="premium-fb-rev-img" src="' . esc_url( $review->reviewer_picture_url ) . '" alt="' . esc_attr( $review->reviewer_name ) . '">';
					} else {
						$author_photo = PREMIUM_FB_REV_AVATAR;
					}
				}
					...
					<div class="premium-fb-rev-review-inner">
						...
						if ( 'bubble' === $skin_type || 'left' === $settings['image_display'] ) {
							...
							<div class="premium-fb-rev-reviewer-header"> <?php } ?>
								<div class="premium-fb-rev-reviewer-wrapper">
									<?php
										$person_link = '<a class="premium-fb-rev-reviewer-link" href="' . $review->url . '" target="_blank"><span class="rating">' . $review->reviewer_name . '</span></a>';
										echo wp_kses_post( $person_link );
									?>
								</div>
								<?php if ( $show_date || $show_stars ) : ?>
									<div class="premium-fb-rev-info">
										<?php if ( $show_date ) : ?>
											<div class="premium-fb-rev-time"><span class="premium-fb-rev-time-text"><?php echo date( $date_format, $review->published_at ); ?></span></div>
										<?php endif; ?>
