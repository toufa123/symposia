...
class Premium_Trustpilot_Reviews extends Widget_Base {
	...
	protected function register_controls() { // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
		...
		$this->add_control(
			...
			array(
				...
				'label_block' => true,
				'description' => sprintf( 'Click <a href="%s" target="_blank">here</a> to get Business ID', Helper_Functions::get_campaign_link( 'https://premiumaddons.com/docs/how-to-get-business-name-for-elementor-trustpilot-reviews-widget/', 'editor-page', 'wp-editor', 'get-support' ) ),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					'1' => __( 'Old', 'premium-addons-pro' ),
					'2' => __( 'New', 'premium-addons-pro' ),
				),
				'default' => '1',
			)
		);
		...
		$this->add_control(
			'trustpilot_api_notice',
			array(
				'raw'             => __( 'Please use the New API if you can\'t see the reviews with the old one. You will need to add the business name manually when the new API is used.', 'premium-addons-pro' ),
		...
		$docs = array(
			...
			'https://premiumaddons.com/docs/how-to-get-business-name-for-elementor-trustpilot-reviews-widget/' => __( 'How to get business ID »', 'premium-addons-pro' ),
	...
	protected function render() {
		...
		if ( false === $response ) {
			...
			$response = premium_trustpilot_data( $business_name, $settings['api'] );
		...
		if ( '2' === $settings['api'] ) {
			$business_settings['name'] = $settings['name'];
		}
			...
			<div class="premium-fb-rev-list">
				...
				<?php if ( ! empty( $reviews ) ) : ?>
					<div <?php echo wp_kses_post( $this->get_render_attribute_string( 'reviews' ) ); ?>>
						...
						<?php
						if ( '1' === $settings['api'] ) {
							premium_trustpilot_old_api_reviews( $reviews, $reviews_settings );
						} else {
							premium_trustpilot_new_api_reviews( $reviews, $reviews_settings );
						}
						?>
					</div>
				<?php endif; ?>
