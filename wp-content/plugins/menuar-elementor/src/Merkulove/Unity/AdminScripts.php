...
final class AdminScripts {
	...
	private function settings_scripts() {
        ...
        wp_enqueue_script( 'mdp-menuar-elementor-ui', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/merkulov-ui' . Plugin::get_suffix() . '.js', [], Plugin::get_version(), true );

        /** Prepare values to pass to JS. */
        $to_js = [
            'ajaxURL'   => admin_url('admin-ajax.php'),
            'nonce'     => wp_create_nonce( 'menuar-elementor' ), // Nonce for security.
        ];

        wp_enqueue_script( 'mdp-menuar-elementor-unity-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
        wp_localize_script( 'mdp-menuar-elementor-unity-admin', 'mdpMenuarElementor', $to_js );
        wp_enqueue_script( 'mdp-menuar-elementor-admin', Plugin::get_url() . 'js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
