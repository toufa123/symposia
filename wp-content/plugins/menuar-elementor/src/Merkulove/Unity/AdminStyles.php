...
final class AdminStyles {
	...
	private function settings_styles() {
		...
		if ( in_array( $screen->base, Plugin::get_menu_bases(), true ) ) {
			...
			wp_enqueue_style( 'mdp-menuar-elementor-ui', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/merkulov-ui.min.css', [], Plugin::get_version() );
            wp_enqueue_style( 'mdp-menuar-elementor-unity-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
			wp_enqueue_style( 'mdp-menuar-elementor-admin', Plugin::get_url() . 'css/admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
	...
	private function plugin_update_styles() {
		...
		if ( isset( $_GET['plugin'] ) && $_GET['plugin'] === 'menuar-elementor' ) {

			wp_enqueue_style( 'mdp-menuar-elementor-plugin-install', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/plugin-install' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
