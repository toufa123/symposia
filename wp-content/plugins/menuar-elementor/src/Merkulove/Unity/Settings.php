...
final class Settings {
        ...
        <aside class="mdc-drawer">
            <div class="mdc-drawer__content">
                <nav class="mdc-list">
                    ...
                    <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Plugin settings', 'menuar-elementor' ) ?></h6>
					<?php
					...
					foreach ( $tabs as $tab => $value ) {
						...
						/** Prepare link. */
						$link = '?page=mdp_menuar_elementor_settings&tab=' . $tab;
	...
	private function support_link() {
	    ...
	    ?>
        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Helpful links', 'menuar-elementor' ) ?></h6>

        <a class="mdc-list-item" href="https://docs.merkulov.design/tag/menuar" target="_blank">
            <i class="material-icons mdc-list-item__graphic" aria-hidden="true">collections_bookmark</i>
            <span class="mdc-list-item__text"><?php echo esc_html__( 'Documentation', 'menuar-elementor' ) ?></span>
        </a>

		<?php if ( TabActivation::get_instance()->is_activated() ) : /** Activated. */ ?>
            <a class="mdc-list-item" href="https://1.envato.market/menuar-elementor-support" target="_blank">
                <i class="material-icons mdc-list-item__graphic" aria-hidden="true">mail</i>
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Get help', 'menuar-elementor' ) ?></span>
            </a>
            <a class="mdc-list-item" href="https://1.envato.market/cc-downloads" target="_blank">
                ...
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Rate this plugin', 'menuar-elementor' ) ?></span>
            </a>
		<?php endif; ?>
        ...
        <a class="mdc-list-item" href="https://1.envato.market/cc-merkulove" target="_blank">
            ...
            <span class="mdc-list-item__text"><?php echo esc_html__( 'More plugins', 'menuar-elementor' ) ?></span>
        </a>
		<?php
	...
	private function add_submenu_elementor() {
        ...
        add_submenu_page(
            $parent,
            esc_html__( 'Menuar Settings', 'menuar-elementor' ),
            esc_html__( 'Menuar for Elementor', 'menuar-elementor' ),
            'manage_options',
            'mdp_menuar_elementor_settings',
            [ $this, 'options_page' ]
        );
    ...
    private function render_nag_saved() {
        ...
        /** Render "Settings Saved" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Settings saved!', 'menuar-elementor' ) );
    ...
    private function render_nag_activation() {
        ...
        if ( TabActivation::get_instance()->is_activated() ) {
            ...
            UI::get_instance()->render_snackbar( esc_html__( 'Plugin activated successfully.', 'menuar-elementor' ), 'success', 5500 );
            return;

        }

        /** Render "Activation failed" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Invalid purchase code.', 'menuar-elementor' ), 'error', 5500 );
        ...
        <div class="mdc-drawer__header mdc-plugin-fixed">
            <a class="mdc-list-item mdp-plugin-title" href="#">
                <i class="mdc-list-item__graphic" aria-hidden="true">
                    <img src="<?php echo esc_attr( Plugin::get_url() . 'images/logo-color.svg' ); ?>" alt="<?php esc_html_e( 'MenuarElementor', 'menuar-elementor' ) ?>">
                </i>
                <span class="mdc-list-item__text">
                    <?php if ( 'wordpress' === Plugin::get_type() ) : ?>
                        <?php esc_html_e( 'Menuar for Elementor', 'menuar-elementor' ) ?>
                    <?php else: ?>
                        <?php esc_html_e( 'Menuar', 'menuar-elementor' ) ?>
                    <?php endif; ?>
                </span>
                <span class="mdc-list-item__text">
                    <sup>
                        <?php esc_html_e( 'v.', 'menuar-elementor' ); ?>
            ...
            <button type="submit" name="submit" id="submit" class="mdc-button mdc-button--dense mdc-button--raised">
                <span class="mdc-button__label"><?php esc_html_e( 'Save changes', 'menuar-elementor' ) ?></span>
            </button>
        </div>
		<?php
	...
	public function get_options() {
        ...
        foreach ( $tabs as $tab_slug => $tab ) {
	        ...
	        $opt_name = "mdp_menuar_elementor_{$tab_slug}_settings";
