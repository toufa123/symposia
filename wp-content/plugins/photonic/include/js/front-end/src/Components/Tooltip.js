...
export const Tooltip = function (selector, tooltip_element) {
	let tooltip, tooltip_class, elm_edges, tooltip_elms;
	...
	function position(tooltip, elm) {
		...
		if (tooltipText !== '') {
			...
			// 10 = arrow height
			const elm_top = elm_edges.top + elm_edges.height + window.scrollY;
			...
			if (elm_edges.left + window.scrollX > viewport_edges && elm_edges.width < 50) {
				...
				const centered = (elm_edges.left + window.scrollX + (elm_edges.width / 2)) - (tooltip.offsetWidth / 2);
