jQuery(document).ready(function($) {
		...
		if (top.wp !== undefined) {
			...
			if (mode === '') {
				...
				photonicNativeWPMediaLibrary.on('select', function() {
					...
					parameters += ((parameters === '') ? '' : '&') + 'action=photonic_wizard_next_screen&screen=' + (activeScreen);
	...
	$(document).on('click', 'a.photonic-flow-more', function(e) {
		...
		if (link !== undefined && link !== '') {
			...
			parameters = 'action=photonic_wizard_more&provider=' + $clicked.data('photonicProvider') + '&display_type=' + $clicked.data('photonicDisplayType') + '&url=' + btoa($clicked.data('photonicMoreLink')) + albumFilter;
			$.post(Photonic_Wizard_JS.ajaxurl, parameters, function(data){
				$clicked.hide();
