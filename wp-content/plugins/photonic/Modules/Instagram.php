...
class Instagram extends OAuth2 implements Level_One_Module {
	...
	public function get_gallery_images($attr = []) {
		global $photonic_instagram_main_size, $photonic_instagram_tile_size, $photonic_instagram_media, $photonic_thumbnail_style, $photonic_instagram_carousel_caption_position;
		...
		if (!empty($media_id)) {// Trumps all else. A single photo will be shown.
			...
			if ($id_format == 'old') {
				$components = [new Error(sprintf(esc_html__('The %s used, %s has been deprecated by Instagram. Please regenerate the gallery with a new id.', 'photonic'), "<code>media_id</code>", "<code>$media_id</code>"))];
				return $components;
			}
			else {
			...
			$query_url = $base_url.$carousel.'?fields='.$this->field_list;
			...
			if (empty($view)) {
				...
				$components = [new Error(sprintf(esc_html__('Malformed shortcode. Either %1$s or %2$s or %3$s has to be defined.', 'photonic'),
		...
		if (!is_wp_error($response)) {
			if (isset($response['response']) && isset($response['response']['code'])) {
				if ($response['response']['code'] == 200) {
					...
					if (isset($body->data) && $display_what == 'media') {
						$data = $body->data;
						$components[] = $this->process_media($data, $shortcode_attr);
					}
					else if ($display_what == 'single-media' || $display_what == 'carousel') {
						...
						if ($body->media_type === 'CAROUSEL_ALBUM') {
							...
							if (!empty($carousel_contents)) {
								if ($shortcode_attr['carousel_caption'] == 'above' && !empty($body->caption)) {
								...
								if ($shortcode_attr['carousel_caption'] == 'below' && !empty($body->caption)) {
									$components[] = "<div class='photonic-caption photonic-instagram-caption'>{$body->caption}</div>";
								}
							}
						}
	...
	function build_level_1_objects($response, array $shortcode, $module_parameters = [], $options = []) {
		...
		foreach ($response as $photo) {
			if (isset($photo->media_type) && (((strtolower($photo->media_type) == 'image' || strtolower($photo->media_type) == 'carousel_album') && $photos_ok) || (strtolower($photo->media_type) == 'video' && $videos_ok)) && isset($photo->media_url)) {
				if (strtolower($photo->media_type) == 'carousel_album' && $shortcode['carousel_handling'] == 'expand') {
	...
	private function process_single_item($photo, $photos_ok, $videos_ok, $photo_link = null, $photo_id = null, $photo_caption = null) {
		if (isset($photo->media_type) && (((strtolower($photo->media_type) == 'image' || strtolower($photo->media_type) == 'carousel_album') && $photos_ok) || (strtolower($photo->media_type) == 'video' && $videos_ok)) && isset($photo->media_url)) {
			...
			if (!empty($photo->thumbnail_url)) {
				$photonic_photo->thumbnail = $photo->thumbnail_url;
				$photonic_photo->tile_image = $photo->thumbnail_url;
			}
			else {
			...
			if (isset($photo->caption)) {
				$photonic_photo->title = esc_attr($photo->caption);
			}
			else if (!empty($photo_caption)) {
				$photonic_photo->title = esc_attr($photo_caption);
			}
			else {
			...
			if (strtolower($photo->media_type) == 'video') {
				$photonic_photo->video = $photo->media_url;
