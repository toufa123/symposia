...
abstract class OAuth2 extends Core {
	...
	public $scope, $response_type, $client_id, $client_secret, $state, $access_token, $auth_error, $soon_limit;
	...
	public abstract function renew_token($token, $save);
	...
	public function authenticate($base_token) {
		...
		if (!isset($photonic_authentication[$this->provider]) && !empty($base_token)) {
			...
			list($token, $error) = $this->renew_token($base_token, true);
		}
		else if (isset($photonic_authentication[$this->provider])) {
			$token = $photonic_authentication[$this->provider];
			if (!empty($token)) {
				if ($this->is_token_expired($token) || $this->is_token_expiring_soon($this->soon_limit) > 0) {
					list($token, $error) = $this->renew_token($base_token, true);
				}
				else {
	 ...
	 */
	abstract function is_token_expiring_soon($soon_limit);
