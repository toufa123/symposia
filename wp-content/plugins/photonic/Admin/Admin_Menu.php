...
class Admin_Menu {
	...
	function enqueue($hook) {
		...
		if ('photonic-options-manager' == $mod_hook) {
			...
			wp_enqueue_script('photonic-options-js', PHOTONIC_URL.'include/js/admin/options-manager.js', ['jquery', 'wp-color-picker'], Photonic::get_version(PHOTONIC_PATH.'/include/js/admin/options-manager.js'));
			wp_localize_script('photonic-options-js', 'Photonic_Options_JS', $js_array);
		}
		else if (in_array($mod_hook, ['photonic-helpers', 'photonic-auth', 'photonic-shortcode-replace'])) {
			wp_enqueue_script('photonic-admin-js', PHOTONIC_URL.'include/js/admin/helpers.js', ['jquery'], Photonic::get_version(PHOTONIC_PATH.'/include/js/admin/helpers.js'));
