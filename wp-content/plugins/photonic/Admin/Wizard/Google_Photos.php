...
class Google_Photos extends Source {
	...
	function make_request($display_type, $for, $flattened_fields) {
		...
		else if ($display_type == 'shared-album-photo') {
			$query_url = 'https://photoslibrary.googleapis.com/v1/sharedAlbums?pageSize=50';
		}
	...
	function deconstruct_shortcode_to_screen_selections($input) {
		...
		if (!empty($input->view)) {
			if ($input->view == 'photos' || $input->view == 'shared-photos') {
				if (!empty($input->album_id)) {
					$deconstructed['display_type'] = ($input->view == 'photos') ? 'album-photo' : 'shared-album-photo';
