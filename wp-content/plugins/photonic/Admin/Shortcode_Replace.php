	...
	function render_content() {
		...
		<form method="post" id="photonic-helper-form" name="photonic-helper-form">
			<div class="photonic-form-body fix">
				...
				<p>
					The <code>gallery</code> shortcode is the backbone of creating galleries in WordPress. By default Photonic
					uses this and adds on a bunch of goodies to it. If you happen to deactivate Photonic, you wouldn't be dealing with
					a mess of shortcodes, since the <code>gallery</code> shortcode fails gracefully and without any ugly errors.
				</p>

				<h2 class="photonic-section">So why might you want to replace it?</h2>
				<p>
					Let's say you have an established website and have made heavy use of Photonic galleries. Then, one fine day,
					you change themes or add some plugin, which also uses the <code>gallery</code> shortcode. You now have a conflict!
					This page gives you a very easy way to work around this conflict.
				</p>
				<p>
					In addition, this method also lets you separate your shortcodes, so that in case you decide to switch over the shortcodes
				...
				?>
				<p>
					You have not defined a shortcode to which you can convert. Please do so from <em>Photonic &rarr; Settings &rarr;
					Generic Options &rarr; Generic settings &rarr; Custom Shortcode</em>.
				</p>
				<?php
				}
				else if (trim($photonic_alternative_shortcode) == 'gallery') {
				?>
				<p>
				...
				<?php
					...
					?>
				<p>
					You have your custom shortcode set up as <code><?php echo $photonic_alternative_shortcode; ?></code>.
				</p>
