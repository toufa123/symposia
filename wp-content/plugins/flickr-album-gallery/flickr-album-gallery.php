...
/*
 * Plugin Name: Album Gallery - Flickr Album Gallery - 2.2.6
...
class FlickrAlbumGallery {
	...
	public function Upgrade_to_meta_box_function() {
		...
		<div class="welcome-panel-column" id="wpfrank-action-metabox">
			...
			<a class="button button-primary button-hero load-customize hide-if-no-customize" target="_blank" href="http://wpfrank.com/account/signup/flickr-album-gallery-pro">Buy Pro Plugin $21</a>
		</div>
		<?php
	}
	...
	public function fag_meta_box_form_function( $post ) {
		...
		<p>
		<input type="radio" name="fag-show-title" id="fag-show-title" value="yes" 
		<?php
		if ( $FAG_Show_Title == 'yes' ) {
			echo esc_attr( 'checked' ); } ?>
		>  <i class="fa fa-check fa-2x"></i> <?php esc_html_e( 'Yes', 'flickr-album-gallery' ); ?>
		<input type="radio" name="fag-show-title" id="fag-show-title" value="no" 
		<?php
		if ( $FAG_Show_Title == 'no' ) {
			echo esc_attr( 'checked' );}
		?>
		>  <i class="fa fa-times fa-2x"></i> <?php esc_html_e( 'No', 'flickr-album-gallery' ); ?>
		</p>
		<br>
		...
		<p>
			<select name="fag-col-layout" id="fag-col-layout" class="fag_layout">
				<optgroup label="<?php esc_html_e( 'Select Column Layout', 'flickr-album-gallery' ); ?>">
					...
					<?php
					if ( $FAG_Col_Layout == 'col-md-4' ) {
						echo esc_attr( 'selected=selected' );
					} ?>
					><?php esc_html_e( 'Three Column', 'flickr-album-gallery' ); ?></option>
					<option value="col-md-3" 
					<?php
					if ( $FAG_Col_Layout == 'col-md-3' ) {
						echo esc_attr( 'selected=selected' );
					}
					?>
					><?php esc_html_e( 'Four Column', 'flickr-album-gallery' ); ?></option>
				</optgroup>
			</select>
		</p>
		<br>
