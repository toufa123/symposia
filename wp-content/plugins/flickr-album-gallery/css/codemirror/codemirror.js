...
(function(mod) {
	...
	function() {
		...
		function CodeMirror(place, options) {
			...
			if (ie && ie_version < 11) {
				setTimeout( function() { cm.display.input.reset( true ); }, 20 );
			}
		...
		TextareaInput.prototype = copyObj(
			{
				...
				fastPoll: function() {
					...
					function p() {
						...
						if ( ! changed && ! missed) {
							missed = true; input.polling.set( 60, p );} else {
		...
		ContentEditableInput.prototype = copyObj(
			{
				...
				showPrimarySelection: function() {
					...
					try {
						var rng = range( start.node, start.offset, end.offset, end.node ); } catch (e) {
				...
				pollContent: function() {
					...
					while (newText.length > 1 && oldText.length > 1) {
						if (lst( newText ) == lst( oldText )) {
							newText.pop(); oldText.pop(); toLine--; } else if (newText[0] == oldText[0]) {
										newText.shift(); oldText.shift(); fromLine++; } else {
								break;
										}
					}
		...
		function registerEventHandlers(cm) {
			...
			if ( ! captureRightClick) {
				on( d.scroller, "contextmenu", function(e) {onContextMenu( cm, e );} );
			}
		...
		function posFromMouse(cm, e, liberal, forRect) {
			...
			try {
				x = e.clientX - space.left; y = e.clientY - space.top; } catch (e) {
				return null; }
		...
		function gutterEvent(cm, e, type, prevent) {
			try {
				var mX = e.clientX, mY = e.clientY; } catch (e) {
							return false; }
		...
		function onFocus(cm) {
			...
			if ( ! cm.state.focused) {
				...
				if ( ! cm.curOp && cm.display.selForContextMenu != cm.doc.sel) {
					cm.display.input.reset();
					if (webkit) {
						setTimeout( function() { cm.display.input.reset( true ); }, 20 ); // Issue #1730
					}
				}
		...
		function markText(doc, from, to, options, type) {
			...
			if (marker.clearOnEnter) {
				on( marker, "beforeCursorEnter", function() { marker.clear(); } );
			}
		...
		Doc.prototype = createObj(
			...
			replaceRange: function(code, from, to, origin) {
				...
				setCursor: docMethodOp(
					function(line, ch, options) {
						setSimpleSelection( this, clipPos( this, typeof line == "number" ? Pos( line, ch || 0 ) : line ), null, options );
					}
				),
			setSelection: docMethodOp(
				function(anchor, head, options) {
					setSimpleSelection( this, clipPos( this, anchor ), clipPos( this, head || anchor ), options );
				}
			),
			extendSelection: docMethodOp(
				function(head, other, options) {
					extendSelection( this, clipPos( this, head ), other && clipPos( this, other ), options );
				}
			),
			extendSelections: docMethodOp(
				function(heads, options) {
					extendSelections( this, clipPosArray( this, heads ), options );
				}
			),
			extendSelectionsBy: docMethodOp(
				function(f, options) {
					var heads = map( this.sel.ranges, f );
					extendSelections( this, clipPosArray( this, heads ), options );
				}
			),
			setSelections: docMethodOp(
				function(ranges, primary, options) {
					if ( ! ranges.length) {
						return;
					}
		...
		var hasSelection   = window.getSelection ? function(te) {
			try {
				return te.selectionStart != te.selectionEnd; } catch (e) {
					return false; }
		} : function(te) {
			try {
				var range = te.ownerDocument.selection.createRange();} catch (e) {
				}
