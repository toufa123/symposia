...
<div id="post-<?php the_ID(); ?>" <?php post_class( 'team-single' ); ?>>
    ...
    <?php endif; ?> 
  <?php  if ( RDTheme::$options['schedule_label_display'] == 'label_meta'  ) {   ?> 

    <?php 
        if( RDTheme::$options['single_event_table']):  
      ...
      // if($difday){
            for($i = 0; $i <= $difday; $i++) {
                ...
                if(!empty($slt)){
                    $inner_content = '<table class="table table-bordered table-responsive">
                                    ...
                                    $count_table = 1;
                                    $tabArray = array();
                                    $inner_contentArray = array();
                                    foreach ($slt as $item) {
                                        if(!in_array($datetime->format('Ymd'), array_keys($tabArray))){
                                            $tabArray[$datetime->format('Ymd')] ='<li class="nav-item '.$tabNall.'">                 
                                                <a class="'.$tabActive.'" href="#schedule-'.$datetime->format('Ymd').'" data-toggle="tab" aria-expanded="true">
                                                    <div class="day-number">'. $item['schedule_label'] .'</div>
                                            ...
                                            $inner_content .='<tr>
                                                ...
                                                $inner_content .='<td>';
                                                   if(!empty($item['speaker'])){
                                                    $inner_content .= '<a href="'.get_the_permalink($item['speaker']).'">'. get_the_title($item['speaker'] ).'</a>';
                                                } 
                                                if(!empty($item['speaker2']) AND !empty($item['speaker'])){
                                                      $inner_content .= ', ';                                                   
                                                }
                                                if(!empty($item['speaker2'])){
                                                       $inner_content .= '<a href="'.get_the_permalink($item['speaker2']).'">'. get_the_title($item['speaker2'] ).'</a>';
                                                }  
                                                $inner_content .='</td>';
                            ...
                            $tabs .= implode('', array_values($tabArray));
                        $inner_content .= '</tbody>
                                    </table>';
                }
        ...
        //}
        printf('<div class="schedule-layout2 schedule-layout5-new">
      ...
      // if($difday){
            for($i = 0; $i <= $difday; $i++) {
                ...
                $tabs .='<li class="nav-item '.$tabNall.'">              
                            <a class="'.$tabActive.'" href="#schedule-'.$datetime->format('Ymd').'" data-toggle="tab" aria-expanded="true">
                                <div class="day-number">Day - 0'.$dayNumber.'</div>
                                    ...
                                    foreach ($slt as $item) {
                                            ...
                                            $inner_content .='<tr>
                                                ...
                                                if(!empty($item['speaker3'])){
                                                       $inner_content .= '<a href="'.get_the_permalink($item['speaker3']).'">'. get_the_title($item['speaker3'] ).'</a>';
                                                }  
                                                ...
                                                if(!empty($item['speaker4'])){
                                                       $inner_content .= '<a href="'.get_the_permalink($item['speaker4']).'">'. get_the_title($item['speaker4'] ).'</a>';
                                                }  
                                                 if(!empty($item['speaker5']) AND !empty($item['speaker4'])){
                                                      $inner_content .= ', ';                                                   
                                                }   
                                                if(!empty($item['speaker5'])){
                                                       $inner_content .= '<a href="'.get_the_permalink($item['speaker5']).'">'. get_the_title($item['speaker5'] ).'</a>';
                                                }  
                                                 if(!empty($item['speaker6']) AND !empty($item['speaker5'])){
                                                      $inner_content .= ', ';                                                   
                                                }
                                                if(!empty($item['speaker6'])){
                                                       $inner_content .= '<a href="'.get_the_permalink($item['speaker6']).'">'. get_the_title($item['speaker6'] ).'</a>';
                                                }  
