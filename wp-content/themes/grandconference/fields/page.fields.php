...
function grandconference_page_create_meta_box() {
	...
	if ( function_exists('add_meta_box') && isset($grandconference_page_postmetas) && count($grandconference_page_postmetas) > 0 ) {  
		add_meta_box( 'page_metabox', 'Page Options', 'grandconference_page_new_meta_box', 'page', 'normal', 'default' );  
	}

}  

function grandconference_page_new_meta_box() {
	$post = grandconference_get_wp_post();
	$grandconference_page_postmetas = grandconference_get_page_postmetas();
	
	//Get page template sample URL
	$page_template_url = array (
	  'Default Template' 						=> grandconference_get_demo_url('/pages/page-fullwidth/'),
	  'Blog Fullwidth' 							=> grandconference_get_demo_url('/blog/blog-fullwidth/'),
	  'Blog Full + Grid Fullwidth' 				=> grandconference_get_demo_url('/blog/blog-full-post-grid-fullwidth/'),
	  'Blog Full + Grid Left Sidebar' 			=> grandconference_get_demo_url('/blog/blog-full-post-grid-left-sidebar/'),
	  'Blog Full + Grid Right Sidebar' 			=> grandconference_get_demo_url('/blog/blog-full-post-grid-right-sidebar/'),
	  'Blog Grid Fullwidth' 					=> grandconference_get_demo_url('/blog/blog-grid-fullwidth/'),
	  'Blog Grid Left Sidebar' 					=> grandconference_get_demo_url('/blog/blog-grid-left-sidebar/'),
	  'Blog Grid Right Sidebar' 				=> grandconference_get_demo_url('/blog/blog-grid-right-sidebar/'),
	  'Blog Left Sidebar' 						=> grandconference_get_demo_url('/blog/blog-left-sidebar/'),
	  'Blog Right Sidebar' 						=> grandconference_get_demo_url('/blog/blog-right-sidebar/'),

	  'Page Left Sidebar' 						=> grandconference_get_demo_url('/pages/page-left-sidebar/'),
	  'Page Right Sidebar' 						=> grandconference_get_demo_url('/pages/page-right-sidebar/'),
	);

	echo '<input type="hidden" name="pp_meta_form" id="pp_meta_form" value="' . wp_create_nonce('grandconference_once') . '" />';
	
	$meta_section = '';
	$key = 0;
	foreach ( $grandconference_page_postmetas as $key => $postmeta ) {
...
function grandconference_page_save_postdata( $post_id ) {
	...
	if ( isset($_POST['pp_meta_form']) && !wp_verify_nonce( $_POST['pp_meta_form'], 'grandconference_once' )) {
		return $post_id;
	}
	...
	if (isset($_POST['pp_meta_form'])) 
	{
		...
		if(is_admin() && isset($_POST['ppb_import_current']) && !empty($_POST['ppb_import_current']))
		{
			...
			else
			{
				...
				{
					$import_options_json = get_option( GRANDCONFERENCE_SHORTNAME."_template_".$_POST['ppb_import_template_key']);
				}
				//If upload import builder file
				else
				{
		...
		foreach ( $grandconference_page_postmetas as $postmeta ) 
		{
		
			if (isset($_POST[$postmeta['id']]) && $_POST[$postmeta['id']]) {
				grandconference_page_update_custom_meta($post_id, $_POST[$postmeta['id']], $postmeta['id']);
			}
		...
		{
			grandconference_page_update_custom_meta($post_id, $_POST['ppb_enable'], 'ppb_enable');
		}
		else
		{
		...
		{
			grandconference_page_update_custom_meta($post_id, $_POST['ppb_form_data_order'], 'ppb_form_data_order');
			...
			if(is_array($ppb_item_arr) && !empty($ppb_item_arr))
			{
				foreach($ppb_item_arr as $key => $ppb_item_arr)
				{
					...
					{
						grandconference_page_update_custom_meta($post_id, $_POST[$ppb_item_arr.'_data'], $ppb_item_arr.'_data');
					}
					
					if(isset($_POST[$ppb_item_arr.'_size']) && !empty($_POST[$ppb_item_arr.'_size']))
					{
						grandconference_page_update_custom_meta($post_id, $_POST[$ppb_item_arr.'_size'], $ppb_item_arr.'_size');
					}
				}
			}
		}
		...
		{
			grandconference_page_update_custom_meta($post_id, '', 'ppb_form_data_order');
		}
	}
	...
	if (isset($_POST['ppb_enable']) && !empty($_POST['ppb_enable']) && ! wp_is_post_revision( $post_id ) )
	{
		...
		remove_action('save_post', 'grandconference_page_save_postdata');
	
		//update the post, which calls save_post again
		$ppb_page_content = grandconference_apply_builder($post_id, 'page', FALSE);
		...
		add_action('save_post', 'grandconference_page_save_postdata');
	}

}

function grandconference_page_update_custom_meta($postID, $newvalue, $field_name) {
...
add_action('admin_menu', 'grandconference_page_create_meta_box'); 
add_action('save_post', 'grandconference_page_save_postdata');  
